import { Cfg, openSheetById, findFolderByPath } from "./DV_Core";
function exportBlob(url: string, name: string) {
  const blob = UrlFetchApp.fetch(url, {
    headers: { Authorization: "Bearer " + ScriptApp.getOAuthToken() }
  }).getBlob().setName(name);
  return blob;
}
export function exportXLSX(cfg: Cfg) {
  ...
  if (!out) throw new Error("Ruta no encontrada para export.");
  const f = out.createFile(blob);
  return f.getUrl();
}
export function exportPDF(cfg: Cfg) {
...
export function exportDashboard(cfg: Cfg) {
  const ss = openSheetById(cfg.SHEET_ID);
  const dash = ss.getSheetByName("Dashboard de conteos");
  if (!dash) throw new Error("No existe Dashboard de conteos");
