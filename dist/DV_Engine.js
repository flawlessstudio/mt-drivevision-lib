...
export function run(cfg, mode) {
    const ss = openSheetById(cfg.SHEET_ID);
    const root = findFolderByPath(cfg.ROOT_PATH);
    if (!root)
        throw new Error("Ruta no encontrada: " + cfg.ROOT_PATH);
    ...
    if (out.length)
        index.getRange(2, 1, out.length, headers.length).setValues(out);
    formatHeader(index);
    if (out.length) {
    ...
    formatHeader(sum);
    if (srows.length > 1)
        numberFmt(sum.getRange(2, 2, srows.length - 1, 1), "0");
